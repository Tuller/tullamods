...
local function GetFormattedTime(secs)
	--day
	if secs >= 86400 then
		return format('%dd', floor(secs/86400 + 0.5)), OmniCC.vlong.s, OmniCC.vlong.r, OmniCC.vlong.g, OmniCC.vlong.b, mod(secs, 86400)
	--hour
	elseif secs >= 3600 then
		return format('%dh', floor(secs/3600 + 0.5)), OmniCC.vlong.s, OmniCC.vlong.r, OmniCC.vlong.g, OmniCC.vlong.b, mod(secs, 3600)
	--3+ minutes
	elseif secs >= 180 then
		return format('%dm', floor(secs/60 + 0.5)), OmniCC.long.s, OmniCC.long.r, OmniCC.long.g, OmniCC.long.b, mod(secs, 60)
	--less than three minutes
	elseif secs >= 60.5 then
		return format("%d:%02d", floor(secs/60), mod(secs, 60)), OmniCC.long.s, OmniCC.long.r, OmniCC.long.g, OmniCC.long.b, secs - floor(secs)
	--seconds, more than 5
...
local function CreateCooldownCount(cooldown, start, duration)
	...
	if icon then
		...
		textFrame:SetAlpha(parent:GetAlpha())
		textFrame:Hide()
