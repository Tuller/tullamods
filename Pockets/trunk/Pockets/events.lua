...
function BagEvents:AddItem(bag, slot)
	...
	self:TriggerMessage('POCKETS_SLOT_ADD', bag, slot, link, count, locked, onCooldown)
end
...
function BagEvents:RemoveItem(bag, slot)
	...
	if data and next(data) then
		...
		self:TriggerMessage('POCKETS_SLOT_REMOVE', bag, slot, prevLink)
	end
end
...
function BagEvents:UpdateItem(bag, slot)
	...
	if data then
		...
		if not(prevLink == link and prevCount == count) then
			...
			self:TriggerMessage('POCKETS_SLOT_UPDATE', bag, slot, link, count, locked, onCooldown)
		end
	end
end
...
function BagEvents:UpdateLock(bag, slot)
	...
	if data and data[1] then
		...
		if data[3] ~= locked then
			data[3] = locked
			self:TriggerMessage('POCKETS_SLOT_UPDATE_LOCK', bag, slot, locked)
		end
	end
end
...
function BagEvents:UpdateCooldown(bag, slot)
	...
	if data and data[1] then
		...
		if data[4] ~= onCooldown then
			data[4] = onCooldown
			self:TriggerMessage('POCKETS_SLOT_UPDATE_COOLDOWN', bag, slot, onCooldown)
		end
	end
end
...
function BagEvents:UpdateBagType(bag)
	...
	if subType ~= bagSubTypes[bag] then
		...
		self:TriggerMessage('POCKETS_BAG_TYPE_CHANGED', bag, type, subType)
	end
end
...
function BagEvents:BANKFRAME_OPENED()
	...
	self:TriggerMessage('POCKETS_BANK_OPENED')
end

function BagEvents:BANKFRAME_CLOSED()
	atBank = false
	self:TriggerMessage('POCKETS_BANK_CLOSED')
end
