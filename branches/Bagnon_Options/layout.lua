...
local function Panel_AddColorPicker(panel, name)
	local function Picker_OnColorChange()
	...
	local function Picker_OnAlphaChange()
		local frame = ColorPickerFrame.frame
	...
	local function Picker_OnCancelChanges()
	...
	local function Picker_OnClick(self)
...
function BagnonOptions:AddLayoutPanel()
	local panel = self:AddPanel(L.Layout)

	local lock = panel:AddCheckButton(L.Lock)
	lock:SetScript("OnClick", function(self) menu.anchor:Lock(self:GetChecked()) end)
	lock:SetScript("OnShow", function(self) self:SetChecked(menu.anchor:GetLocked()) end)

	local reverse = panel:AddCheckButton(L.ReverseSort)
	reverse:SetScript("OnClick", function(self) SetReverseSort(menu.anchor, self:GetChecked()) end)
	reverse:SetScript("OnShow", function(self) self:SetChecked(menu.anchor:GetLocked()) end)

	local topLevel = panel:AddCheckButton(L.Toplevel)
	topLevel:SetScript("OnClick", function(self) SetToplevel(menu.anchor, self:GetChecked()) end)
	topLevel:SetScript("OnShow", function(self) self:SetChecked(menu.anchor:GetLocked()) end)

	local topLevel = panel:AddCheckButton(L.Toplevel)
	topLevel:SetScript("OnClick", function(self) SetToplevel(menu.anchor, self:GetChecked()) end)
	topLevel:SetScript("OnShow", function(self) self:SetChecked(menu.anchor:GetLocked()) end)

	local colorPicker = Panel_AddColorPicker(panel, L.BackgroundColor)
	colorPicker:SetPoint("TOPLEFT", panel.button, "BOTTOMLEFT", 4, 2)
	panel.height = panel.height + 32
